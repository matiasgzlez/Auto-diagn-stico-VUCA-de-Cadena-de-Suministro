...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ result, onReset }) => {
  ...
  useEffect(() => {
    // Enviar datos a Google Forms cuando se muestra la pantalla de resultados
    if (result.userData) {
      submitToGoogleForms(result.userData, result)
        .then((success) => {
          setDataSubmitted(success);
            ...
            <div className="flex items-start space-x-4">
              {isSubmitting ? (
                <>
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900 mb-1">Enviando tu reporte...</h3>
                    <p className="text-blue-700 text-sm">Estamos procesando tus datos y preparando tu reporte completo.</p>
                  </div>
                </>
              ) : dataSubmitted ? (
                <>
                  ...
                  <div>
                    <h3 className="font-semibold text-green-900 mb-1">¡Reporte enviado exitosamente!</h3>
                    <p className="text-green-700 text-sm">
                      Tu reporte completo ha sido enviado a <strong>{result.userData.correo}</strong>. 
                ...
                <>
                  ...
                  <div>
                    <h3 className="font-semibold text-yellow-900 mb-1">Error al enviar el reporte</h3>
                    <p className="text-yellow-700 text-sm">
                      No pudimos enviar tu reporte automáticamente. Por favor, contáctanos directamente en{' '}
                      <a href={`mailto:${K2_CONTACT.email}`} className="underline font-semibold">{K2_CONTACT.email}</a> para recibir tu reporte.
                    </p>
                  </div>
                </>
              )}
